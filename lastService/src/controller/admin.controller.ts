...
export default class adminController {
    ...
    async getBuyTransAction(req: Request, res: Response, next: NextFunction){
        ...
        let transActions;
        if (type == 0) {
            transActions = await queryBuilder.where('invoice.status = :status AND type.title = :title AND invoice.fromPhone = :boolean', { status: 'failed', title: 'buy' , boolean : false}).getMany()
        } else if (type == 1) {
            transActions = await queryBuilder.where('invoice.status = :status AND type.title = :title AND invoice.fromPhone = :boolean', { status: 'completed', title: 'buy' , boolean : false}).getMany()
        } else if (type == 2) {
            transActions = await queryBuilder.where('invoice.status = :status AND type.title = :title AND invoice.fromPhone = :boolean', { status: 'pending', title: 'buy' , boolean :  false}).getMany()
        }else if(type == 3){
            transActions = await queryBuilder.where('invoice.status = :status AND type.title = :title AND invoice.fromPhone = :boolean', { status: 'init', title: 'buy' , boolean : false }).getMany()
        }else{
