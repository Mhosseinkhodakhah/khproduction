...
parentPort.on('message', (data) => {
    ...
    if (data == 'first task'){
        setInterval(()=>{
            console.log('first task is runnign' , `${isMainThread} , ${threadId}`)
        } , 1000)
    }else if (data == 'second task'){
        setInterval(()=>{
            console.log('second task is runnign' , `${isMainThread} , ${threadId}`)
        } , 1000)        
    }
    else if (data == 'thirdTask task'){
        setInterval(()=>{
            console.log('third task is runnign' , `${isMainThread} , ${threadId}`)
        } , 1000)        
    }
