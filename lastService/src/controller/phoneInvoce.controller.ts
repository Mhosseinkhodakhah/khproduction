    ...
    private async estimateWeight(goldWeight: number, type: number) {
        try {
            if (type == 0) {
                ...
                let monthEstimate = await this.estimate.findOne({where : {
                    month : month
                }})
                if (monthEstimate){
                    monthEstimate.soldGold = (parseFloat(((+monthEstimate.soldGold) + goldWeight).toFixed(3))).toString()
                    await this.estimate.save(monthEstimate)
                }else{
                    let newMonth = await this.estimate.create({month : month , boughtGold : '0' , soldGold : (parseFloat(((+monthEstimate.soldGold) + goldWeight).toFixed(3))).toString()})
                    await this.estimate.save(newMonth)
                }
                let estimate2 = await this.estimate.findOne({
                    where: {
                ...
                if (estimate2) {
                    estimate2.soldGold = (parseFloat(((+estimate2.soldGold) + goldWeight).toFixed(3))).toString()
            ...
            if (type == 1) {
                ...
                if (monthEstimate){
                    monthEstimate.boughtGold = (parseFloat(((+monthEstimate.boughtGold) + goldWeight).toFixed(3))).toString()
                    await this.estimate.save(monthEstimate)
                }else{
                    let newMonth =  this.estimate.create({month : month , boughtGold : (parseFloat(((+monthEstimate.boughtGold) + goldWeight).toFixed(3))).toString() , soldGold : '0'})
                    await this.estimate.save(newMonth)
                }
                ...
                await this.estimate.save(totalEstimate)
                if (estimate2) {
                    estimate2.boughtGold = (parseFloat(((+estimate2.boughtGold) + goldWeight).toFixed(3))).toString()
                    await this.estimate.save(estimate2)
                } else {
                    let estimate2 = this.estimate.create({
                        date: new Date().toLocaleString("fa-IR").split(",")[0],
